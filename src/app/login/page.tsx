...
function LoginForm() {
  ...
  const oauthError = searchParams.get("error"); // NextAuth OAuth error
  ...
  // Если пользователь уже авторизован — перенаправить со страницы входа
  useEffect(() => {
  ...
  // Показать ошибку OAuth если есть
  useEffect(() => {
    if (oauthError) {
      const errorMessages: Record<string, string> = {
        OAuthCallback: "Ошибка при входе через Google. Попробуйте ещё раз.",
        OAuthSignin: "Ошибка при подключении к Google.",
